...
void* ramfs_init(void) {
  ...
  ramfs_instance.files[0].in_use = true;
  ramfs_instance.files[0].is_directory = true;
  strcpy(ramfs_instance.files[0].path, "/");
  return &ramfs_instance;
}
...
static RamFSFile* create_file(RamFS *fs, const char *path, bool is_dir) {
  for (int i = 0; i < RAMFS_MAX_FILES; i++) {
    if (!fs->files[i].in_use) {
      ...
      fs->files[i].is_directory = is_dir;
      fs->files[i].size = 0;
      strcpy(fs->files[i].path, path);
      return &fs->files[i];
    }
  }
  return NULL;
...
static void* ramfs_open(void *fs_data, const char *path, unsigned mode) {
  ...
  // Allocate handle
  static RamFSHandle handles[MAX_OPEN_FILES];
...
static int ramfs_readdir(void* fs_data, void* handle, char* buffer, size_t size) {
  ...
  for (unsigned i = 0; i < RAMFS_MAX_FILES; i++) {
    if (fs->files[i].in_use) {
      size_t path_len = strlen(fs->files[i].path);
      if ((written + path_len) < size) {
        strcpy(buffer + written, fs->files[i].path);
        written += path_len;
      }
      if (written < size) {
        buffer[written] = ' ';
